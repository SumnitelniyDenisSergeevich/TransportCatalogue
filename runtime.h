...
namespace runtime {
    ...
    template <typename Type, typename Сomparator>
    ObjectHolder CompareObjectHolders(const ObjectHolder& lhs, const ObjectHolder& rhs) {
        if (lhs.TryAs<ValueObject<Type>>() && rhs.TryAs<ValueObject<Type>>()) {
            return ObjectHolder::Own(runtime::Bool{ Сomparator{}(lhs.TryAs<ValueObject<Type>>()->GetValue(), rhs.TryAs<ValueObject<Type>>()->GetValue()) });
        }
        return {};
    }
