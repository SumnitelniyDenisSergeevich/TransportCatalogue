...
public:
	void AddRoute(const Bus& bus, const std::vector<std::string> stops);
...
private:
	...
	std::unordered_map<const Stop*, std::set<std::string_view>> stop__by_buses_; // доделать, достать из Stop, и освободить там
